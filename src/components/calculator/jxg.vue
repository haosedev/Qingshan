<template>
    <div>
        ...
        <group :title="title">
         ...
         <group :title="itemStr">
            <x-input title="*边长(D)" v-model="val_l" type="number" pattern="[0-9][.]*" :show-clear="false"><div slot="right" mini>mm</div></x-input>
        ...
        <div v-show="showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    ...
                    <p>2.理计重量<br/>W=0.00002491*｛（l+d）*2÷3.14-s｝*s*L</p>
                </div>
            </card>
        </div>
         <group v-show="!showTab1">
        ...
        <div v-show="!showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    <p>1.本工具计算的理论重量与实际重量有出入，误差约为0.2%-0.7%，计算结果仅估算参考，不建议在实际交易中使用。</p>
                    <p>2.理计长度<br/>L=W/0.00002491*｛（l+d）*2÷3.14-s｝*s</p>
                </div>
            </card>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            res1() {
                //var totalWeight = density*((Number(wSide)+Number(wWidth))*2/3.14-wThickness)*wThickness*wLength;
                var key_num = this.itemList.indexOf(this.item[0]);
                var density = this.numList[key_num];
                if (density > 0) {
                    //W=0.00002491*｛（l+d）*2÷3.14-s｝*s*L
                    var result = density * ((parseFloat(this.val_l) + parseFloat(this.val_d)) * 2 / 3.14 - parseFloat(this.val_s)) * parseFloat(this.val_s) * parseFloat(this.val_ll);
                    return result;
                } else {
                    return 0;
                }
            ...
            res2() {
                //var totalWidth = lWeight/(density*((Number(lSide)+Number(lWidth))*2/3.14-lThickness)*lThickness);
                var key_num = this.itemList.indexOf(this.item[0]);
                var density = this.numList[key_num];
                if (density > 0) {
                    //L=W/0.00002491*｛（l+d）*2÷3.14-s｝*s
                    var result = parseFloat(this.val_ww) / (density * ((parseFloat(this.val_l) + parseFloat(this.val_d)) * 2 / 3.14 - parseFloat(this.val_s)) * parseFloat(this.val_s));
                    return result;
                } else {
                    return 0;
                }
