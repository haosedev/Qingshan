<template>
    <div>
        ...
        <group :title="title">
         ...
         <group :title="itemStr">
            <x-input title="*外径(D)" v-model="val_d" type="number" pattern="[0-9][.]*" :show-clear="false"><div slot="right" mini>mm</div></x-input>
        ...
        <div v-show="showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    ...
                    <p>2.理计重量<br/>W=0.00002491*S（D-S）*L</p>
                </div>
            </card>
        </div>
         <group v-show="!showTab1">
        ...
        <div v-show="!showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    <p>1.本工具计算的理论重量与实际重量有出入，误差约为0.2%-0.7%，计算结果仅估算参考，不建议在实际交易中使用。</p>
                    <p>2.理计长度<br/>L=W/0.00002491*S(D-S)</p>
                </div>
            </card>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            res1() {
                //var totalWeight = density * wWidth * (wDiameter - wWidth) * wLength;
                var key_num = this.itemList.indexOf(this.item[0]);
                var density = this.numList[key_num];
                if (density > 0) {
                    //W=0.00002491*S（D-S）*L
                    var result = density * parseFloat(this.val_s) * (parseFloat(this.val_d) - parseFloat(this.val_s)) * parseFloat(this.val_ll);
                    return result;
                } else {
                    return 0;
                }
            ...
            res2() {
                //var totalWidth = lWeight/(density*lWidth*(lDiameter-lWidth));
                var key_num = this.itemList.indexOf(this.item[0]);
                var density = this.numList[key_num];
                if (density > 0) {
                    var result = parseFloat(this.val_ww) / (density * parseFloat(this.val_s) * (parseFloat(this.val_d) - parseFloat(this.val_s)));
                    return result;
                } else {
                    return 0;
                }
