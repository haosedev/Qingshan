    ...
    <div>
        ...
        <div v-show="showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    ...
                    <p>2.理计重量<br/>W=0.00000793*[h*d+2*t(b-d)+0.349(R*R-r*r)]*L</p>
                </div>
            </card>
        </div>
         <group v-show="!showTab1">
        ...
        <div v-show="!showTab1">
            ...
            <card>
                <div slot="content" class="card-padding">
                    <p>1.本工具计算的理论重量与实际重量有出入，误差约为0.2%-0.7%，计算结果仅估算参考，不建议在实际交易中使用。</p>
                    <p>2.理计长度<br/>L=W/0.00000793*[h*d+2*t(b-d)+0.349(R*R-r*r)]</p>
                </div>
            </card>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            res1() {
                //var totalWeight = density*(Number(wHeight*wWaist)+Number(2*wAverageLeg*(wLeg-wWaist))+0.349*(wInRadius*wInRadius-wOutRadius*wOutRadius))*wLength;
            ...
            res2() {
                //var totalWidth = lWeight/(density*(Number(lHeight*lWaist)+Number(2*lAverageLeg*(lLeg-lWaist))+0.349*(lInRadius*lInRadius-lOutRadius*lOutRadius)));
                var key_num = this.itemList.indexOf(this.item[0]);
                var density = this.numList[key_num];
                if (density > 0) {
