<template>
    <div>
        ...
        <group :title="title">
         ...
         <group :title="itemStr">
            <x-input title="*对边距离(s)" v-model="val_s"  type="number" pattern="[0-9][.]*" :show-clear="false"><div slot="right" mini>mm</div></x-input>
         </group>
    ...
    export default {
        ...
        computed: {
            res1() {
                ...
                if (density > 0) {
                    //W = 0.00000669 * s * s * L
                    var result = density * parseFloat(this.val_s) * parseFloat(this.val_s) * parseFloat(this.val_ll);
                    return result;
                } else {
                    return 0;
                }
            ...
            res2() {
                ...
                if (density > 0) {
                    //L = W / 0.00000669 * s * s
                    var result = parseFloat(this.val_ww) / (density * parseFloat(this.val_s) * parseFloat(this.val_s));
                    return result;
                } else {
                    return 0;
                }
